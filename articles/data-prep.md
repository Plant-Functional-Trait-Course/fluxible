...
This is annoying because `read_delim` will read that row as data, and
therefore the column with gas concentration will be read as character
instead of numeric. Normally, those units are stated in the protocol or
(draft) data paper of the project, so it is ok to remove them. There are
